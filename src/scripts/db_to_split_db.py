"""Read an ase.db file and split indices and write a new database for each split."""
...
def main(args):
    ...
    assert Path(args.splits).is_file(), f"{args.splits} is not a file."
    # Setup job
    ...
    for k,v in splits.items():
        assert isinstance(v, list), f"Indices for split '{k}' must be a list of integers."
        assert len(v) == len(set(v)), f"Indices for split '{k}' must be unique."
    # Write database for each split
    for split_name, indices in splits.items():
        split_dir = output_dir / split_name.replace(" ", "_").lower()
        split_dir.mkdir(parents=True, exist_ok=True)
        N = len(indices)  # Number of rows to write
        n = 0  # Number of rows written
        logging.info(f"Processing split '{split_name}' with {N} rows...")
        with ase.db.connect(args.asedb) as input_db:
            while n < N:  # While there are still rows to write
                ...
                if args.rows_per_file < N:
                    ...
                    output_name = (
                        f"data_{part:02}.aselmdb" if args.lmdb else f"data_{part:02}.db"
                    )
                    output_path = split_dir / output_name
                else:
                    # Single output file
                    output_name = "data.aselmdb" if args.lmdb else "data.db"
                    output_path = split_dir / output_name
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Read an ase.db file and split indices and write a new database for each split."
    )
    parser.add_argument("--asedb", help="Path to ASE database input file.", required=True)
    parser.add_argument(
        "--splits", help="Path to JSON file with split names and indices.", required=True
    )
